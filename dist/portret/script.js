...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunction = function calcFunction() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      resultBlock.textContent = 'Заполните поля!!!';
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      ...
      if (fileInputs[0].value !== '') {
        ...
        Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["postData"])('assets/question.php', input.files[0]).then(function (res) {
          console.log(res);
...
var filter = function filter() {
  ...
  var optimizationCode = function optimizationCode(elem, mark) {
    elem.addEventListener('click', function () {
      typeFilter(mark);
    });
  };

  optimizationCode(btnAll, markAll);
  optimizationCode(btnLovers, markLovers);
  optimizationCode(btnChef, markChef);
  optimizationCode(btnGirl, markGirl);
  optimizationCode(btnGuy, markGuy);
  optimizationCode(btnGranddad, '');
  optimizationCode(btnGrandmother, '');
...
var modals = function modals() {
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      ...
      if (!display) {
        ...
        document.body.style.overflow = 'hidden';
        document.querySelector(selector).classList.add('anime');
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      var scrollHeight = Math.max(document.documentElement.scrollHeight - 1, document.body.scrollHeight - 1);
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function hideImg(block) {
    ...
    block.querySelectorAll('p:not(.size)').forEach(function (p) {
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResource"])(
    /* 'http://localhost:3000/styles' */
    'assets/db.json').then(function (res) {
      return createCards(res.styles);
    }).catch(function (error) {
      return (
        /* console.log('error') */
        createErrorImg(error)
      );
    });
    this.remove();
  });
  ...
  function createErrorImg() {
    ...
    var statusImg = document.createElement('img');
    statusImg.setAttribute('scr', message.fail);
    statusImg.classList.add('animated', 'fadeInUp');
    statusImg.cssText = 'display: block; margin: 0 auto;';
    document.querySelector('#styles .row').appendChild(statusImg);
  }
